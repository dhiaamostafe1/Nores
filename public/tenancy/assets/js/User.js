    ...
    $(document).ready(function(){
      ...
      $.ajaxSetup({
        ...
        //           }
        //          var div1 = '<tr id="EntryType'+ data.inf.id+'"><th scope="row">'+data.inf.id +'</th> <td id="EntryTypename'+data.inf.id+'">'+data.inf.TAK_NAME+'</td> ';
        //          var div2 = '<td>';
     ...
     // if(confirm('do ready to delete')){
         ...
         $.ajax({
              ...
              success: function (req) {
                $('#nameedit').val(req.count.name);
                $('#emailedit').val(req.count.email);
                $('#passwordedit').val(req.count.password);
                $('#iduser').val(req.count.id);
                   console.log(req);
              }
          });
