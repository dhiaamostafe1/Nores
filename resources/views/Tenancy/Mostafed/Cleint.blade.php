    ...
    <div class="card-header" style="padding: 8px 14px 0px 14px">
        <div class="row">
            <div class="col-lg-9 col-sm-6 col-xs-6 mb-2">
                <h4 class="d-flex flex-row">{{__('aside.Cleint')}}</h4>
            </div>
            <div class="col-lg-3 col-sm-6 col-xs-6 ">
             
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('Client.print')}}" class="d-flex fa fa-file-pdf-o  fa-2x flex-row-reverse p-1" ></a>
                    <a href="{{route('Client.excel')}}" class="d-flex fa fa-file-excel-o fa-2x flex-row-reverse p-1" ></a>
                    <i class="d-flex fa fa-upload fa-2x p-1 flex-row-reverse"  data-bs-toggle="modal"  data-bs-target="#InportBankModel"></i>
                    @if (getrull(session()->get('loginuser'), 29,'Inter'))
                ...
                <table class="table table-hover table-borderless">
                    ...
                    <tbody  id="uniteItem">
                        ...
                        <tr id="deleteitemClient{{$item->id }}">
                            ...
                            <td>{{$item->ACC_NO }}</td>
                            {{-- <td>{{$item->Kindly }}</td> --}}
                            @if (getrull(session()->get('loginuser'), 29,'Update'))
                            <td>  <a type="button" class=" ti-pencil editeMostafed" id="delete"  data-contant="{{$item->id }}"></a></td>
                            @endif
                            @if (getrull(session()->get('loginuser'), 29,'Delete'))
